...
public class BinPacking3D_LAFF {
	...
	public void packLevel() {
		...
		for (Box box : boxes) {
			int surfaceArea = box.dimension.get("length") * box.dimension.get("breadth");
			if (surfaceArea > surfaceAreaBiggest) {
				boxBiggest = box;
				surfaceAreaBiggest = surfaceArea;
			}
		}
		containerDimension.put("height", containerDimension.get("height") + boxBiggest.dimension.get("height"));
		System.out.println("container Height: " + containerDimension.get("height"));
		...
		if (boxes.isEmpty())
			return;
		Integer containerArea = containerDimension.get("length") * containerDimension.get("breadth");
		Integer boxArea = boxBiggest.dimension.get("length") * boxBiggest.dimension.get("breadth");

		if (containerArea - boxArea > 0) {
			if (containerDimension.get("length") - boxBiggest.dimension.get("length") > 0) {
				fillSpace(new Box(containerDimension.get("length") - boxBiggest.dimension.get("length"), containerDimension.get("breadth"), boxBiggest.dimension.get("height")), level);
			}
			if (containerDimension.get("breadth") - boxBiggest.dimension.get("breadth") > 0) {
				fillSpace(new Box(boxBiggest.dimension.get("length"), containerDimension.get("breadth") - boxBiggest.dimension.get("breadth"), boxBiggest.dimension.get("height")), level);
			}
    ...
    //		boxes.add(new Box("1", 50, 50, 22));
    //		boxes.add(new Box("2", 33, 8, 14));
    //		boxes.add(new Box("3", 16, 20, 8));
    //		boxes.add(new Box("4", 3, 18, 24));
    //		boxes.add(new Box("5", 14, 2, 8));
    //		boxes.add(new Box("6", 35, 10, 8));
    //		boxes.add(new Box("7", 99, 12, 8));
    //		boxes.add(new Box("8", 19, 25, 30));
    //
